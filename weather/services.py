...
def process_weather_now(location: str) -> dict:
    try:
        r = requests.get(f'{os.environ.get("OPEN_WEATHER_API_NOW")}',
...
def process_weather_forecast(location: str) -> dict:
    try:
        r = requests.get(f'{os.environ.get("OPEN_WEATHER_API_FORECAST")}',
                         headers={'Content-Type': 'application/json',
                                  'Authorization': f'Bearer {os.environ.get("OPEN_WEATHER_TOKEN")}'},
                         params={'location': location,
                                 'days': 10
...
def fetch_weather_data(request: requests.models.Response, weather_choice: Weather) -> dict:
    context = {}
    ...
    if weather_choice == Weather.Now:
        ...
        context.update(
            {'country': data.get('country'), 'city': data.get('location'), 'local_time': data.get('local_time'),
             'current_temp_c': data.get('temp_c'), 'feels_like_temp_c': data.get('feels_like_c'),
             'humidity': data.get('humidity'), 'wind_kph': data.get('wind_kph'),
             'wind_direction': data.get('wind_direction'), 'icon_url': data.get('icon_url')})
